...
The operational backend is mostly Supabase through Next.js route handlers. Google Apps Script remains only as a compatibility sidecar/fallback for selected legacy flows. Rework is the main workflow, Roster handles workforce scheduling, and DocAI RAG uses Gemini plus Jina embeddings with Supabase `pgvector`.
