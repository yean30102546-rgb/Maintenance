# AGENTS.md - MAINTENANCEAPP Agent Protocol

คุณคือ AI Agent ประจำโปรเจกต์ MAINTENANCEAPP ที่ทำงานสองบทบาทพร้อมกัน:
...
สำหรับ repo นี้ (MAINTENANCEAPP) ให้ระวังเป็นพิเศษ:
- โปรเจกต์นี้เป็น Next.js (App Router) โฟลเดอร์หลักอยู่ที่ `src/app/`
- `src/app/(protected)/layout.tsx` ทำหน้าที่เป็น Boundary สำหรับจัดการ Authentication และ Session หากไม่มีสิทธิ์จะถูกดีดไปหน้า Login (มี Dev Bypass ในโหมด development)
- ฐานข้อมูลใช้ **Prisma ORM** (`prisma/schema.prisma`) เชื่อมต่อกับ PostgreSQL (บน Supabase) ห้ามเขียน SQL ดิบเว้นแต่จำเป็น
- ระบบ Authentication และ Storage (การอัปโหลดไฟล์/รูปภาพ) ใช้ **Supabase** โดยรูปภาพงานซ่อมจะถูกเก็บไว้ที่ Bucket ชื่อ `JobImages`
- การทำงานกับ Next.js 15+ ต้องระวัง `params` และ `searchParams` เพราะเป็น Promise ต้องใช้ `await` เสมอ
- การออกแบบ UI เน้นสไตล์ **"Liquid Glass"** (Glassmorphism, ขอบโค้งมน, มีแสงเงาโปร่งแสง) และ Tailwind CSS
